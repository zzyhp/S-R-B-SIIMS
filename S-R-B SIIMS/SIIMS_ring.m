...
N1=129; N2=129; 
data=load('O-Oms-u.mat');

%%%%%%%%%%%%%%%%%%%parameter%%%%%%%%%%%%%
%% All other parameters remain unchanged; only parameter kexi=0;0.01;0.02;0.03;0.04 is adjusted.
K=2;beta2=0.01;K1=18;K2=19;zeta=1;kexi=0.04;gamma=4;ITE=100;mu2=0.000001;beta1=0.014;mu1=0.000001;
T=data.T;D=data.D;kappa0=-0.001;kappa=0.001;lambda1=0.15;lambda2=0.01;eta1=0.01;eta2=0.0001;
%%%%%%%%%%%%%%%%%%%parameter%%%%%%%%%%%%%
...
figure
 ...
 imshow(TTS,[0,255]);
 title('registration and bias correction result')
...
TTSS=zeros(N1,N2);
  for i=2:128
    for j=2:128
 TTSS(i,j)=(abs(TT(i,j)-s(i,j)))/exp(m(i,j));
    end
  end
 imshow(TTSS,[0,255]);
 title('image with only Bias_correction')
...
hold on
for i=1:3:129
plot(phy_y(i,1:129),phy_x(i,1:129),'b');
hold on
end
hold on
for j=1:3:129
plot(phy_y(1:129,j),phy_x(1:129,j),'b');
hold on
end
axis equal
axis([1,129,1,129])
...
figure
imshow(TTk,[0,255]);
title('image with only registration')

figure
  imagesc(zeros)
colormap(gray)
title('pure deformation')
axis equal
axis ij
hold on
for i=1:3:129
